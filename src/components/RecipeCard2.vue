<template>
	<article class="card">
		<div class="thumb"><img :src="this.image" alt="People" style="no-repeat; center;" /></div>
		<div class="infos">
			<h2 class="title">{{ this.title }}</h2>
			<div class="txt" style="height:70%; overflow-y: auto; overflow-x: hidden;">
				<div v-bind:key="x" v-for="x in this.instructions">{{ x }}</div>
			</div>
			<h3 class="details">View</h3>
		</div>
	</article>
</template>
	...
	methods: {
		...
		toggleOverlay() {
			...
			if (this.overlay_on) {
				...
				document.getElementById('overlay_instructions').innerHTML += this.instructions;
			} else {
